...
UdpServer::UdpServer()
{
    ...
    this->p = Polynom<TComplex>();
    this->d = Polynom<double>();
}
...
{    
    ...
    if (packet[0] == "CALC")
    {
        ...
        if (packet[7] == "complex") {
            ...
            TComplex a, b, c;
            a = TComplex(packet[1].toDouble(), packet[2].toDouble());
            b = TComplex(packet[3].toDouble(), packet[4].toDouble());
            c = TComplex(packet[5].toDouble(), packet[6].toDouble());

            p = Polynom<TComplex>(a, b, c);
            ...
            d = Polynom<double>(a, b, c);

            double x[2];
            int res = d.roots(x);
            if (res == 2 || res == 1) {
                datagram = QString("ROOTS:X1:%1:X2:%2")
                        .arg(QString::number(x[0]))
                        .arg(QString::number(x[1])).toUtf8();
                std::cout << "Send ROOTS\n";
            }
            if (res == 0) {
                datagram = QString("ERR:[Server] Equation has no roots.").toUtf8();
    ...
    if (packet[0] == "SHOW")
    {
        QByteArray datagram;
        if (packet[1] == "complex")
            datagram = (QString("VIEW:p(x) = %1 * x^2 + %2 * x + %3")
                        .arg(numberToChar(p.A()).constData())
                        .arg(numberToChar(p.B()).constData())
                        .arg(numberToChar(p.C()).constData())).toUtf8();
        else datagram = (QString("VIEW:d(x) = %1 * x^2 + %2 * x + %3")
                         .arg(QString::number(d.A()))
                         .arg(QString::number(d.B()))
                         .arg(QString::number(d.C())).toUtf8());
                this->writeDatagram(datagram, QHostAddress::LocalHost, 1919);
        qDebug() << datagram;
        std::cout << "Send VIEW\n";
        return;
...
QByteArray UdpServer::numberToChar(TComplex a)
{
    QByteArray ba;
    cout << a << endl;
    if (a.real() != 0) {
